...
require BASE . '/app/Mage.php';

if (!Mage::isInstalled())
{
    Throw New \RuntimeException ('Application is not installed yet, please complete install wizard first.');
}
...
try
{
    ...
    {
        ini_set('error_log', $_SERVER['HOME'] . '/InteractiveConsoleLog.log');
    }

    error_reporting(E_ALL);
    Mage::app('','store');

    InteractiveConsole::getInstance()->read();
}
...
Class InteractiveConsole
{
    ...
    protected function __construct()
    {
        ...
        if (! empty($_SERVER['HOME']))
        {
            ...
            if (! file_exists($this->historyFile)) file_put_contents($this->historyFile, '');
    ...
    {
        if (SIGINT === $signal|| SIGTERM === $signal) $this->quit();
    }

    public function __destruct()
    {
        if (! empty($this->historyFile) && is_writable($this->historyFile)) readline_write_history($this->historyFile);
    }
    ...
    public static function getInstance()
    {
        ...
        {
            self::$instance = New InteractiveConsole();
        }
    ...
    protected function completeCallback($line)
    {
        if (! empty($line))
        {
            ...
            $avail = array_merge(get_declared_classes(),$funcs['user'], $funcs['internal'], []);
            ...
            if (substr($line, -4) == '\:\:')
            {
                ...
                if (in_array($class, $avail))
                {
                    ...
                    foreach ($methods as $key => $method) $methods[$key] = $class.'::'.$method;
